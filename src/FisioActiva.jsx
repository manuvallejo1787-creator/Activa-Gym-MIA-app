...
export default function FisioActiva({ brand, gymClients=[], onUpdateGymClient, reglas=[] }){
  ...
  const AltasCli=()=>{
    ...
    const exportAltaPDF=()=>{
      ...
      <style>${getPrintCSS(NV,TL)}</style></head>
      ...
      ${last.antrop&&(last.antrop.peso||last.antrop.talla)?`<div class="sec"><div class="sec-title">Antropometría</div><div class="sec-body" style="display:grid;grid-template-columns:repeat(3,1fr);gap:6px;font-size:11px">${last.antrop.peso?'<div>Peso: <strong>'+last.antrop.peso+' kg</strong></div>':''}${last.antrop.talla?'<div>Talla: <strong>'+last.antrop.talla+' cm</strong></div>':''}${last.antrop.imc||last.antrop.peso&&last.antrop.talla?'<div>IMC: <strong>'+(last.antrop.imc||(parseFloat(last.antrop.peso)/Math.pow(parseFloat(last.antrop.talla)/100,2)).toFixed(1))+'</strong></div>':''}${last.antrop.perCintura?'<div>Cin: <strong>'+last.antrop.perCintura+' cm</strong></div>':''}${last.antrop.perCadera?'<div>Cad: <strong>'+last.antrop.perCadera+' cm</strong></div>':''}${last.antrop.pliegues?'<div style="grid-column:1/-1"><strong>Pliegues:</strong> '+last.antrop.pliegues+'</div>':''}</div></div>`:''}
  ...
  const VerEval=()=>{
    ...
    const exportPDF=()=>{
      ...
      const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Evaluación ${pac?.nombre}</title><style>${getPrintCSS(NV,TL)}</style></head>
      ...
      ${Object.keys(ev.rom||{}).length>0?`<div class="sec"><div class="sec-title">ROM Goniometría</div><div class="sec-body filas">${(ROM_NORMS[ev.region]||ROM_NORMS.lumbar).filter(n=>romGrados(ev.rom,n.mov)).map(n=>{const v=romGrados(ev.rom,n.mov);const romObj=(typeof ev.rom[n.mov]==='object'&&ev.rom[n.mov])||{};const p=n.normal>0?Math.round(parseFloat(v)/n.normal*100):0;const col=romObj.funcional===false?'#DC2626':(p>90?'#16A34A':p>70?'#D97706':'#DC2626');const extra=[romObj.funcional===true?'✓ funcional':romObj.funcional===false?'⚠ disfuncional':'',romObj.eva!==undefined&&romObj.eva!==''?`EVA ${romObj.eva}/10`:''].filter(Boolean).join(' · ');return`<div class="fila"><span class="fila-label">${n.mov}${extra?` <span style="font-weight:400;color:#888;font-size:10px">— ${extra}</span>`:''}</span><span class="fila-val" style="color:${col}">${v}° <span style="font-weight:400;color:#999;font-size:10px">/ ${n.normal}° (${p}%)</span></span></div>`}).join('')}</div></div>`:''}
      ...
      ${ev.criterios_personalizados?.length>0?`<div class="sec"><div class="sec-title">Criterios de Evolución Personalizados</div><div class="sec-body">${ev.criterios_personalizados.map(c=>`<div class="criterio"><span class="arrow">→</span>${c}</div>`).join('')}</div></div>`:''}
      <div class="footer">FisioActiva Colonia · Método Activa Integra · ${new Date().toLocaleDateString('es-ES')}</div>
      <script>window.onload=()=>{window.print()}<\/script></body></html>`;
      const w=window.open('','_blank');w.document.write(html);w.document.close();
    };
